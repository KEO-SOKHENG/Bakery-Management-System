    ...
    @forelse($recipes as $recipe)
        <div class="recipe-card">
            ...
            <div class="recipe-card-footer">
                <div>
                    <span class="recipe-cost-label">Est. Unit Cost</span>
                    <span class="recipe-cost-value">${{ number_format($recipe->estimated_cost, 2) }}</span>
                </div>
    ...
    <div class="dash-modal-content">
        ...
        <form action="{{ route('admin.recipes.store') }}" method="POST" style="display: flex; flex-direction: column; gap: 1rem;">
            ...
            <div>
                ...
                <input type="text" name="recipe_name" class="ios-input-field" placeholder="e.g. Classic Butter Croissant Formula" required>
            </div>
            <div>
                <label class="form-label">Target Bakery Product</label>
                <select name="product_name" class="ios-select-field" required>
                    @foreach($products as $p)
                        <option value="{{ $p->name }}">{{ $p->name }} ({{ $p->category_name }})</option>
                    @endforeach
                </select>
            </div>
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 0.75rem;">
                <div>
                    ...
                    <input type="number" name="yield_quantity" class="ios-input-field" placeholder="50" required>
                </div>
                <div>
                    <label class="form-label">Prep Time</label>
                    <input type="text" name="prep_time" class="ios-input-field" placeholder="45 Mins" required>
                </div>
                <div>
                    <label class="form-label">Bake Temp</label>
                    <input type="text" name="bake_temp" class="ios-input-field" placeholder="210°C" required>
                </div>
            </div>
            <div>
                <label class="form-label">Est. Cost Per Unit ($)</label>
                <input type="number" step="0.01" name="estimated_cost" class="ios-input-field" placeholder="1.25" required>
            </div>
            <div>
                <label class="form-label">Ingredients Formula Summary</label>
                <textarea name="ingredients_summary" class="ios-textarea-field" rows="2" placeholder="e.g. 10kg T55 Flour, 6.5kg Normandy Butter, 500g Sugar, 250g Yeast, 180g Salt" required></textarea>
            </div>
            <div>
                <label class="form-label">Baking Instructions & Notes</label>
                <textarea name="baking_instructions" class="ios-textarea-field" rows="2" placeholder="Fermentation steps, dough lamination notes, and oven humidity settings..."></textarea>
            </div>
